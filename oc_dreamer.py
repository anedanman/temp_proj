...
from oc_models import OC_RSSM, OC_ConvEncoder, OC_ConvDecoder, OC_DenseDecoder, OC_ActionDecoder
from utils import *
...
class Dreamer:
    ...
    def _build_model(self, restore):
        ...
        self.actor = OC_ActionDecoder(
                     ...
                     n_layers=3,
                     activation=self.args.dense_activation_function).to(self.device)
        self.obs_encoder  = OC_ConvEncoder(
                            input_shape= self.obs_shape,
                            num_blocks=self.args.nblocks,
                            in_channels=self.args.in_channels,
                            hidden_size=self.args.hidden_size,
                            slot_size = self.args.obs_embed_size,
                            num_slots =self.args.nslots).to(self.device)
        self.obs_decoder  = OC_ConvDecoder(
                            stoch_size = self.args.stoch_size,
                            deter_size = self.args.deter_size,
        ...
        if self.args.use_disc_model:  
          self.discount_model = OC_DenseDecoder(
                                stoch_size = self.args.stoch_size,
                                deter_size = self.args.deter_size,
                                output_shape = (1,),
                                n_layers = 2,
    ...
    def world_model_loss(self, obs, acs, rews, nonterms):
        ...
        obs_dist, _ = self.obs_decoder(features)
        if self.args.use_disc_model:
    ...
    def evaluate(self, env, eval_episodes, render=False):
        ...
        imagine_images = [[] for _ in range(eval_episodes)]
        slotvise = [[] for _ in range(eval_episodes)]
            ...
            while not done:
                with torch.no_grad():
                    ...
                    if prior is None:
                        ...
                        img_action = self.actor(torch.cat([prior['stoch'], prior['deter']], dim=-1))
                        prior = self.rssm.imagine_step(prior, img_action)
                    img_obs, recons = self.obs_decoder(torch.cat([prior['stoch'], prior['deter']], dim=-1))
                    img_obs = (img_obs.mean.cpu().numpy()[0] + 0.5)*255.0
                    recons = (recons[0].cpu().numpy() + 0.5)*255.0
                    # print('img_obs shape', img_obs.shape)
                    # print('obs shape', obs['image'].shape)
                    imagine_images[i].append(img_obs.copy().clip(0, 255).astype(np.uint8))
                    slotvise[i].append(recons.copy().clip(0, 255).astype(np.uint8))
                action = action[0].cpu().numpy()
                next_obs, rew, done, _ = env.step(action)
                prev_state = posterior
                prev_action = torch.tensor(action, dtype=torch.float32).to(self.device).unsqueeze(0)
                ...
                if render:
                    video_images[i].append(obs['image'].copy())
                obs = next_obs
            if render:
                video_images[i] = np.stack(video_images[i])
                imagine_images[i] = np.stack(imagine_images[i])
        return episode_rew, video_images, imagine_images, slotvise
...
def main():
    ...
    # Data parameters
    parser.add_argument('--max-episode-length', type=int, default=2000, help='Max episode length')
    parser.add_argument('--buffer-size', type=int, default=2000000, help='Experience replay size')  # Original implementation has an unlimited buffer size, but 1 million is the max experience collected anyway
    parser.add_argument('--time-limit', type=int, default=2000, help='time limit') # Environment TimeLimit
    # Models parameters
    ...
    parser.add_argument('--model_learning-rate', type=float, default=2e-4, help='World Model Learning rate') 
    parser.add_argument('--actor_learning-rate', type=float, default=4e-5, help='Actor Learning rate') 
    parser.add_argument('--value_learning-rate', type=float, default=1e-4, help='Value Model Learning rate')
    ...
    parser.add_argument('--test-interval', type=int, default=20000, help='Test interval (episodes)')
    ...
    parser.add_argument('--max-videos-to-save', type=int, default=4, help='max_videos for saving')
    ...
    train_env = Atari(f"ALE/{args.env}-v5", grayscale=False)
    test_env  = Atari(f"ALE/{args.env}-v5", grayscale=False)
    obs_shape = train_env.observation_space.shape
    action_size = train_env.action_space.shape[0]
    dreamer = Dreamer(args, obs_shape, action_size, device, args.restore)

    logger = Logger(logdir)
    wandb.init(project='dreaming', name=args.exp_name)
    ...
    if args.train:
        ...
        while global_step <= args.total_steps:
            ...
            if global_step % args.test_interval == 0:
                episode_rews, video_images, imagine_images, slots = dreamer.evaluate(test_env, args.test_episodes, render=True)
                
                wandb.log(
                    {
                        'env_acting': [wandb.Video(video_images[i], fps=24) for i in range(4)],
                        'imagine_acting': [wandb.Video(imagine_images[i], fps=24) for i in range(4)]
                    },
                     step=global_step
                )
                wandb.log(
                    {
                        f'SLOT {s}': [wandb.Video(slots[i, s]) for i in range(4)] 
        ...
        logs = OrderedDict()
        episode_rews, video_images, _ = dreamer.evaluate(test_env, args.test_episodes, render=True)

        logs.update({
            'TEST/avg_reward':np.mean(episode_rews),
            'TEST/max_reward': np.max(episode_rews),
            'TEST/min_reward': np.min(episode_rews),
            'TEST/td_reward':np.std(episode_rews),
        })

        wandb.log(
            {'TEST/env_acting': [wandb.Video(video_images[i]) for i in range(video_images.shape[0])]}, 
            step=global_step
            )
